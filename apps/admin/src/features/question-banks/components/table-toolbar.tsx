...
interface Props {
    table: Table<QuestionBank>;
}

export function QuestionBanksTableToolbar({ table }: Props) {
    const globalFilter = (table.getState() as any).globalFilter ?? '';
    const setGlobalFilter = table.setGlobalFilter;

    const minGradeCol = table.getColumn('minGrade');
    const maxGradeCol = table.getColumn('maxGrade');
    const activeCol = table.getColumn('isActive');
    ...
    const hasFilters =
        !!globalFilter ||
        !!activeCol?.getFilterValue() ||
        !!createdCol?.getFilterValue();

    const reset = () => {
        setGlobalFilter('');
        activeCol?.setFilterValue(undefined);
        setFrom('');
        setTo('');
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-72 space-y-3 p-3">
                        <div className="space-y-1">
                            ...
                            <Select
                                value={(activeCol?.getFilterValue() as string) ?? 'all'}
                                onValueChange={(v) => {
                                    if (!activeCol) return;
                                    if (v === 'all') activeCol.setFilterValue(undefined);
                                    if (v === 'active') activeCol.setFilterValue(true);
                                    if (v === 'archived') activeCol.setFilterValue(false);
                                }}
                            >
                                <SelectTrigger>
