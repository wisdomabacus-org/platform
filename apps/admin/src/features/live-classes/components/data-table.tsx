...
import { LiveClassesTableToolbar } from './table-toolbar';
...
export function LiveClassesDataTable<TData, TValue>({
  columns,
  data,
  pageSize = 10,
}: Props<TData, TValue>) {
  ...
  const table = useReactTable({
    ...
    filterFns: {
      withinRange: (row, columnId, filter) => {
        if (!filter || (typeof filter === 'object' && !filter.from && !filter.to)) return true;
  ...
  return (
    <div className="space-y-4">
      <LiveClassesTableToolbar table={table as any as import('@tanstack/react-table').Table<LiveClassInfo>} />
