...
import { useCompetition, useCompetitionQuestionBanks, useAssignQuestionBanks } from '../hooks/use-competitions';
...
export default function CompetitionQuestionsPage() {
  const { id: competitionIdParam } = useParams<{ id: string }>();
  const { data: competition, isLoading: isLoadingComp } = useCompetition(competitionIdParam!);
  const { data: assignedBanks, isLoading: isLoadingAssigned } = useCompetitionQuestionBanks(competitionIdParam!);
  const { data: allBanks, isLoading: isLoadingAllBanks } = useQuestionBanks();
  const { mutate: assignBanks, isPending: isSaving } = useAssignQuestionBanks();
  ...
  const handleSave = () => {
    ...
    assignBanks({ id: competitionIdParam!, assignments: validAssignments });
  };

  if (isLoadingComp || isLoadingAssigned || isLoadingAllBanks) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-6xl mx-auto">
      <PageHeader
        title={`Question Banks - ${competition?.title}`}
        description="Assign question banks and grades for this competition."
      >
